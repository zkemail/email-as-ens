...
contract HandleRegistrar is IEntryPoint {
     ...
     * @dev Verifies the proof using ClaimHandleCommandVerifier, deploys account if not exists, and withdraws ETH
     * @param data Encoded ClaimHandleCommand containing proof, public inputs, and target address
    ...
    function claimAndWithdraw(bytes memory data) public returns (address account) {
        ...
        // Convert x handle to ENS node: handle.x.zkemail.eth
