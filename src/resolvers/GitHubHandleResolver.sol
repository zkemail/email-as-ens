...
interface IGitHubHandleRegistrar {
...
contract GitHubHandleResolver is IExtendedResolver, Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    /// @param _registrar The address of the GitHubHandleRegistrar contract
    ...
    function resolve(bytes calldata name, bytes calldata data) external view override returns (bytes memory) {
        ...
        if (selector == IAddrResolver.addr.selector) {
            ...
            if (registrar != address(0)) {
                address account = IGitHubHandleRegistrar(registrar).getAccount(node);
                // If account exists, return it; otherwise return predicted address
                if (account != address(0)) {
                    return abi.encode(account);
                }
                // Return predicted address
                return abi.encode(IGitHubHandleRegistrar(registrar).predictAddress(node));
            }
