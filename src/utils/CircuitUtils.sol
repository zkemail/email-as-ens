...
library CircuitUtils {
     ...
     */
    function packString(string memory _string, uint256 paddedSize) internal pure returns (bytes32[] memory fields) {
        uint256[] memory fieldsArray = packBytes2Fields(bytes(_string), paddedSize);
     ...
     * @return fields The packed field element
     */
    function packBool(bool b) internal pure returns (bytes32[] memory fields) {
        fields = new bytes32[](1);
        fields[0] = b ? bytes32(uint256(1)) : bytes32(uint256(0));
        return fields;
    }
    ...
    {
        uint256[] memory _pucSignals = new uint256[](_pubSignals.length);
        for (uint256 i = 0; i < _pubSignals.length; i++) {
            _pucSignals[i] = uint256(_pubSignals[i]);
        }
        uint256 remain = _paddedSize % 31;
        uint256 numFields = (_paddedSize - remain) / 31;
        if (remain > 0) {
     ...
     */
    function unpackBool(bytes32[] calldata pubSignals, uint256 startIndex) internal pure returns (bool) {
        return uint256(pubSignals[startIndex]) == 1;
    }
    ...
    function flattenFields(uint256[][] memory inputs, uint256 outLength) internal pure returns (uint256[] memory out) {
        ...
        for (uint256 i = 0; i < inputs.length; i++) {
            ...
            for (uint256 j = 0; j < arr.length; j++) {
                if (k >= outLength) revert InvalidPublicInputsLength();
                out[k++] = arr[j];
            }
        }
        if (k != outLength) revert InvalidPublicInputsLength();
