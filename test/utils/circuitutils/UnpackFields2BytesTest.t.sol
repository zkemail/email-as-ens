...
contract UnpackWrapper {
    ...
    {
        return CircuitUtils.unpackFields2Bytes(fields, startIndex, numFields, paddedSize);
    }
}

contract UnpackFields2BytesTest is Test {
    UnpackWrapper wrapper;

    function setUp() public {
        wrapper = new UnpackWrapper();
    }

    function test_emptyFields() public view {
        uint256[] memory fields = new uint256[](0);
        bytes memory result = wrapper.unpackFields2Bytes(fields, 0, 0, 0);
    ...
    function test_singleFieldSingleByte() public view {
        ...
        fields[0] = 0x41;
        bytes memory result = wrapper.unpackFields2Bytes(fields, 0, 1, 1);
    ...
    function test_singleFieldMultipleBytes() public view {
        ...
        bytes memory result = wrapper.unpackFields2Bytes(fields, 0, 1, 3);
    ...
    function test_multipleFields() public view {
        ...
        bytes memory result = wrapper.unpackFields2Bytes(fields, 0, 2, 6);
    ...
    function test_withOffset() public view {
        ...
        bytes memory result = wrapper.unpackFields2Bytes(fields, 1, 1, 3);
        assertEq(result.length, 3);
    ...
    function test_moreFieldsThanAvailable() public view {
        ...
        bytes memory result = wrapper.unpackFields2Bytes(fields, 0, 1, 4);
        assertEq(result.length, 2);
        assertEq(uint8(result[0]), 0x41);
        assertEq(uint8(result[1]), 0x42);
    }
    ...
    function test_allZeros() public view {
        uint256[] memory fields = new uint256[](1);
        fields[0] = 0;
        bytes memory result = wrapper.unpackFields2Bytes(fields, 0, 1, 31);
        assertEq(result.length, 0);
    }
    ...
    function test_multipleFieldsWithPadding() public view {
        ...
        bytes memory result = wrapper.unpackFields2Bytes(fields, 0, 2, 5);
        // Only the first 3 bytes are non-zero, the rest are zeros and will be trimmed
        assertEq(result.length, 3);
        assertEq(uint8(result[0]), 0x41);
        assertEq(uint8(result[1]), 0x42);
    ...
    function test_partialFieldUnpack() public view {
        ...
        bytes memory result = wrapper.unpackFields2Bytes(fields, 0, 1, 2);
        assertEq(result.length, 2);
        assertEq(uint8(result[0]), 0x41);
        assertEq(uint8(result[1]), 0x42);
    }
